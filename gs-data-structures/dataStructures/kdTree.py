...
class KdTree:
    def createKdTree(values, depth=0):
        ...
        right = KdTree.createKdTree(
            values[(median + 1) :], depth + 1
        )  # median + 1 because I store a value in the node
    ...
    def NNS(currentNode, point, bestNode, bestDist, distCalc):
        ...
        distCalc += 1
        axisDistance = currentNode.value.axisDist(point, currentNode.axis)
        ...
        (bestNode, bestDist, distCalc) = KdTree.NNS(
            near, point, bestNode, bestDist, distCalc
        )
