...
def multipleNNSearches(elements, args, print_results = True):
    ...
    all_NNS = [NNSearch(elements,args,print_results=False) for i in range(args.number_of_runs)]
...
def _split_elements(elems, args):
    ...
    if(args.number_of_searches == 0 or args.number_of_searches + num_elem_in_tree > len(elements)):
        args.number_of_searches = len(elements)-num_elem_in_tree
    
    tree_elements = elements[0:num_elem_in_tree]
    search_elements = elements[num_elem_in_tree:num_elem_in_tree+args.number_of_searches]
...
def NNSearch(elements, args, print_results = True):
    ...
    tree = VPTree.createVPTree(tree_elements,random=args.random_vp, max_leaf_size = args.leaf_size)
    
    start_time = time.time()
    NNS = [VPTree.nearestNeighbour(tree, elem, args.k, args.greedy_factor) for elem in search_elements]
...
def generatePointTree(args):
    numberList = [VPTreeElement(np.random.uniform(args.min_value, args.max_value, args.dim )) for x in range(args.number_of_num_searches)]
    tree = VPTree.createVPTree(numberList, args.random_vp, args.leaf_size)
    return(tree)

def number_NN(tree, args):
    elements = [VPTreeElement(np.random.uniform(args.min_value,args.max_value, args.dim)) for x in range(args.number_of_num_searches)]
...
parser.add_argument("--no_nn_test", action='store_true',help= "Should a nearest neighbour test be performed")
...
parser.add_argument("--number_of_num_searches", type=int, default=100, help="Number of DIM dimensional points to search for nearest neighbours with")
parser.add_argument("--greedy_factor",type=float, default=1, help="Determines how greedy the pruning is. A solution must be atleast this much better to be considered. Default is 1, that is, anything which has the ability to be better is considered. 2 means the solution has to be atleast twice as good to be considered")
...
parser.add_argument("--number_of_searches",type=int,default=0,help="how many of the elements should be used to NN search in the tree?")

parser.add_argument("--greedy_test",action='store_true', help="This will run multiple runs with different greedy factors determined by --greedy_start, --greedy_end, --greedy_step")
parser.add_argument("--greedy_start",type=float,default=1,help="start value of greedy_factor when running greedy__test")
