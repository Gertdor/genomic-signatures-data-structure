...
def x(orig_matrix, orig_names, other_matrix, other_names, args):
    ...
    found_organisms = {name:orig_names[neighbor] for name,neighbor in zip(orig_names,orig_neighbors)}
    other_organisms = {name:other_names[neighbor] for name,neighbor in zip(other_names,other_neighbors)}
...
parser.add_argument(
    "--original_matrix",help="path to original distance matrix"
)
parser.add_argument(
    "--other_matrix",help="path to other distance matrix"
)
parser.add_argument(
    "--k",default=1,help="number of neighbors"
)
